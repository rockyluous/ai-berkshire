...
若 `reports/{公司名}/00-数据底稿.json` 存在，先 `datasheet --check`（美股 `usstock_data.py` / A股 `ashare_data.py` / 台股 `twstock_data.py`），过期则刷新；股价与最新季报数字以底稿为准，不要各自实时拉价。其余用 WebSearch 收集：
1. 最新财报数据（如果有新的季报/年报）
...
将本次检查记录追加到 `reports/{公司名}/{公司名}-thesis.md` 的追踪记录表中，并同步更新「检验点日历」（删掉已检的，补下一个财报日 / 事件日）。之后 `python3 tools/thesis_calendar.py --company {公司名}` 应不再报逾期：
