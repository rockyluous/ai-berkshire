...
3. team-lead 把四份异议汇成 `reports/{公司名}/00-交叉质询.md`（表：条目 | 提出方 | 异议方 | 依据 | 裁决），裁决连同依据写进最终报告第 10 节复核记录
...
lint 的「过程日期晚于今天」（FUTURE-DATE）只看建立/核验/生成/基准/更新这类"什么时候做的"日期——实测一轮研究把建立日期和 12 条核验日期全写成了次日，thesis_calendar 与索引照单全收，根因是拿了 UTC 日期（美西下午 5 点后 UTC 已是次日）。开工前先跑 `date`（本地时间），所有过程日期用它。检验点日历、财报日这类将来日期不受限。
...
四条命令任一报错，先查清楚谁错了再汇总——**尤其 crosscheck 报不一致时，不要取平均、不要选看起来顺眼的那个，要回到算式看谁漏了因子**。crosscheck 报「口径未标」不是算错，是让该角色补括号口径后重跑（此时工具打【待补口径】而不是【通过】，补齐前不能汇总——实测 20 倍错在没标口径时就长这样）；报「口径不同」时在最终报告并列两种口径并注明。字典内指标"仅单算"的要补算，字典外的不用管。
...
> 放在第 2 节是为了让读者先看到结论和可执行价位；但**写作顺序不变**——价格区间必须来自第 8 节前瞻价值区间表与三情景估值，先算后写，本节只引用不另算。
